...
void HAL_TIM_PeriodElapsedCallback(TIM_HandleTypeDef* tim_baseHandle)
{
	...
	if(tim_baseHandle->Instance == htim1.Instance)
	{
		...
		if(query_encoder_speed_count >= 40)	//40ms
		{
			...
			/*输入死区*/
			if (fabs(Motor_PID.PID_Error0) < 5)		//如果误差绝对值比较小
			{
				Motor_PID.PWM_Out = 0;				//则直接输出0，不进行调控
			}
			else
			{
				/*误差积分（累加）*/	
				/*
					误差积分+积分分离 
					这个阈值根据PD实际测试的差值，阈值稍微大于最终的差值。
				*/
				if (fabs(Motor_PID.PID_Error0) < 50)							//如果当前误差值小于指定的阈值
				{
					Motor_PID.PID_ErrorInt += Motor_PID.PID_Error0;		//才进行正常的误差积分
				}
				else						//否则，即当前误差值过大
				{
					Motor_PID.PID_ErrorInt = 0;			//此时不进行积分，同时把误差积分直接清零
				}
				...
				/*输出偏移*/
				if (Motor_PID.PWM_Out > 0.1)		//如果输出为正
				{
					Motor_PID.PWM_Out += 6;		//则直接给输出值加上一个固定偏移
				}
				else if (Motor_PID.PWM_Out < -0.1)	//如果输出为负
				{
					Motor_PID.PWM_Out -= 6;		//则直接给输出值减去一个固定偏移
				}
				else				//输出为0
				{
					Motor_PID.PWM_Out = 0;		//输出0
				}	
			}
