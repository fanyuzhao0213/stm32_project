...
void NRF24L01_W_CE(uint8_t BitValue)
{
	...
	HAL_GPIO_WritePin(NRF_CE_PORT, NRF_CE_PIN, (GPIO_PinState)BitValue);
}
...
void NRF24L01_Send_IT(uint8_t *pData, uint8_t len)
{
    ...
    /* 写入待发送数据 */
    NRF24L01_WriteTxPayload(pData, len);

    /* 进入发送模式，NRF24L01 会自动在发送完成后触发中断 */
    NRF24L01_Tx();

    printf("[TX] Start sending %d bytes...\r\n", len);
}
...
uint8_t NRF24L01_TestHardware(void)
{
    ...
    printf("=== NRF24L01 硬件自检开始 ===\r\n");

    // 1. 读取 STATUS 寄存器
    status = NRF24L01_ReadStatus();
    printf("初始 STATUS 寄存器值: 0x%02X\r\n", status);

    if (status == 0xFF || status == 0x00)
    {
        printf("[错误] STATUS=0x%02X，SPI 无法正常通信，请检查接线和电源！\r\n", status);
        return 1;
    }

    // 2. 测试 RF_CH 寄存器写入和读取
    NRF24L01_WriteReg(NRF24L01_RF_CH, test_value);  // 写入 0x55
    read_value = NRF24L01_ReadReg(NRF24L01_RF_CH);  // 读出来

    printf("写入 RF_CH=0x%02X，读取 RF_CH=0x%02X\r\n", test_value, read_value);

    if (read_value != test_value)
    {
        printf("[错误] 寄存器读写失败，SPI 接线或 NRF24L01 模块异常！\r\n");
        return 2;
    }
