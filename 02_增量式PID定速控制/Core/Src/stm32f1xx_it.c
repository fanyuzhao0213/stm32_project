...
void HAL_TIM_PeriodElapsedCallback(TIM_HandleTypeDef* tim_baseHandle)
{
	...
	if(tim_baseHandle->Instance == htim1.Instance)
	{
		...
		if(query_encoder_speed_count >= 40)	//40ms
		{
			...
			/*获取本次误差、上次误差和上上次误差*/
			Motor_PID.PID_Error2 = Motor_PID.PID_Error1;							//获取上上次误差
			...
			Motor_PID.PWM_Out += Motor_PID.PID_Kp * Motor_PID.PID_Error0 + \
						...
						/*PID计算*/
			/*使用增量式PID公式，计算得到输出值*/
			Motor_PID.PWM_Out += Motor_PID.PID_Kp  * (Motor_PID.PID_Error0 - Motor_PID.PID_Error1) + \
								 Motor_PID.PID_Ki * Motor_PID.PID_Error0 + \
								 Motor_PID.PID_Kd * (Motor_PID.PID_Error0 - 2 * Motor_PID.PID_Error1 + Motor_PID.PID_Error2);
