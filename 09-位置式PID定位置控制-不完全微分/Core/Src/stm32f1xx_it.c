...
void HAL_TIM_PeriodElapsedCallback(TIM_HandleTypeDef* tim_baseHandle)
{
	...
	if(tim_baseHandle->Instance == htim1.Instance)
	{
		...
		if(query_encoder_speed_count >= 40)	//40ms
		{
			...
			Motor_PID.PID_Actual += rand() % 41 - 20;
			...
			float a = 0.9;
			Motor_PID.PID_DifOut = (1 - a) * Motor_PID.PID_Kd * (Motor_PID.PID_Error0 - Motor_PID.PID_Error1) + a * Motor_PID.PID_DifOut;		//这一句是不完全微分的微分项计算公式
