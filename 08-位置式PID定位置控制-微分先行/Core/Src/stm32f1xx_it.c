...
void HAL_TIM_PeriodElapsedCallback(TIM_HandleTypeDef* tim_baseHandle)
{
	...
	if(tim_baseHandle->Instance == htim1.Instance)
	{
		...
		if(query_encoder_speed_count >= 40)	//40ms
		{
			...
			Motor_PID.PID_Actual1 = Motor_PID.PID_Actual;			//获取上次实际值
			Motor_PID.PID_Actual += Encoder_Get();					//获取本次实际值
...
//			//微分项输出值
//			Motor_PID.PID_DifOut = Motor_PID.PID_Kd * (Motor_PID.PID_Error0 - Motor_PID.PID_Error1);
			
			//这一句是微分先行的微分项计算公式
			//计算结果要取负，因为实际值的变化趋势和误差变化趋势相反
			Motor_PID.PID_DifOut = - Motor_PID.PID_Kd  * (Motor_PID.PID_Actual - Motor_PID.PID_Actual1);		
