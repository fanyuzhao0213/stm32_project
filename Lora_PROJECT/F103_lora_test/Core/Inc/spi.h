...
/* USER CODE BEGIN Prototypes */
HAL_StatusTypeDef	SPI2_TransmitOneByte(uint8_t	byteData);
//SPI接口发送多个字节, pBuffer是发送数据缓存区指针，byteCount是发送数据字节数，byteCount最大256
HAL_StatusTypeDef	SPI2_TransmitBytes(uint8_t* pBuffer, uint16_t byteCount);
//SPI接口接收一个字节， 返回接收的一个字节数据
uint8_t	SPI2_ReceiveOneByte(void);
//SPI接口接收多个字节， pBuffer是接收数据缓存区指针，byteCount是需要接收数据的字节数
HAL_StatusTypeDef	SPI2_ReceiveBytes(uint8_t* pBuffer, uint16_t byteCount);
