...
{
    hcan1.Init.Mode = CAN_MODE_LOOPBACK;   // 设置为回环模式
    ...
    if (HAL_CAN_Start(&hcan1) != HAL_OK)    // 启动 CAN 外设
    {
        Error_Handler();
    }
...
void CAN_Send_Test_Frames(void)
{
	...
	rtt_printf("CAN Frame %d sent! ID: 0x%X\r\n", can_test_data_index, 
...
void HAL_CAN_RxFifo0MsgPendingCallback(CAN_HandleTypeDef *hcan)
{
    ...
    if (rxHeader.IDE == CAN_ID_STD)
        rtt_printf("CAN message received: StdID=0x%03X DLC=%d", rxHeader.StdId, rxHeader.DLC);
    else
        rtt_printf("CAN message received: ExtID=0x%08X DLC=%d", rxHeader.ExtId, rxHeader.DLC);
