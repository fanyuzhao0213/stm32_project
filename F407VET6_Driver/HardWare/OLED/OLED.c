...
#define OLED_I2C_ADDR        (0x3C << 1)  // OLED I2C 地址左移1位（写模式）
/**
  * 函    数：OLED写命令
...
{
    uint8_t buf[2];
    buf[0] = 0x00;       // 控制字节，0x00 表示写命令
    buf[1] = Command;    // 要写入的命令

    HAL_I2C_Master_Transmit(&hi2c1, 0x78, buf, 2, HAL_MAX_DELAY);  // 发送地址和数据
}
...
void OLED_WriteData(uint8_t *data, uint8_t count)
{
    ...
    if (buf == NULL) return;  // 分配失败保护

    buf[0] = 0x40;  // 控制字节：数据
    memcpy(&buf[1], data, count);  // 拷贝数据内容
