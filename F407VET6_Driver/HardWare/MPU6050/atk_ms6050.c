...
//        *temp = (int16_t)((36.53f + ((float)raw / 340)) * 100);
        *temp = (int16_t)((19 + (((float)raw-521) / 340)) * 100);
...
{
    uint8_t id;
    if (atk_ms6050_read_byte(ATK_MS6050_IIC_ADDR, MPU_DEVICE_ID_REG, &id) != ATK_MS6050_EOK)
    {
        rtt_printf("MPU6050 self check failed: I2C error\r\n");
        return MPU6050_ERR;
    }

    if (id != 0x70)
    {
        rtt_printf("MPU6050 self check failed: ID=0x%02X\r\n", id);
        return MPU6050_ERR;
    }

    rtt_printf("MPU6050 self check OK, ID=0x%02X\r\n", id);
    return MPU6050_OK;
}
...
void mpu6050_calibrate(void)
{
    ...
    rtt_printf("MPU6050 calibrating...\r\n");
    ...
    mpu_offset.gyro_offset_x = gx_sum / count;
    mpu_offset.gyro_offset_y = gy_sum / count;
    mpu_offset.gyro_offset_z = gz_sum / count;

    mpu_offset.accel_offset_x = ax_sum / count;
    mpu_offset.accel_offset_y = ay_sum / count;
    mpu_offset.accel_offset_z = az_sum / count;

    rtt_printf("Calibration done: Gx=%d Gy=%d Gz=%d, Ax=%d Ay=%d Az=%d\r\n",
               mpu_offset.gyro_offset_x, mpu_offset.gyro_offset_y, mpu_offset.gyro_offset_z,
               mpu_offset.accel_offset_x, mpu_offset.accel_offset_y, mpu_offset.accel_offset_z);
}
...
void mpu6050_demo_run(void)
{
	...
	if (atk_ms6050_init() != ATK_MS6050_EOK)
    {
        rtt_printf("MPU6050 init failed!\r\n");
        return;
    }
	
    if (mpu6050_self_check() != MPU6050_OK) return;


    if (atk_ms6050_dmp_init() != MPU6050_OK)
    {
        rtt_printf("MPU6050 DMP init failed!\r\n");
        return;
    }
    ...
    {
        // 获取 DMP 数据
        ret = atk_ms6050_dmp_get_data(&pitch, &roll, &yaw);
        atk_ms6050_get_accelerometer(&ax,&ay,&az);
        atk_ms6050_get_gyroscope(&gx,&gy,&gz);
        atk_ms6050_get_temperature(&temp);

        if (ret == ATK_MS6050_EOK)
        {
            // 减去偏置
            gx -= mpu_offset.gyro_offset_x;
            gy -= mpu_offset.gyro_offset_y;
            gz -= mpu_offset.gyro_offset_z;

            ax -= mpu_offset.accel_offset_x;
            ay -= mpu_offset.accel_offset_y;
